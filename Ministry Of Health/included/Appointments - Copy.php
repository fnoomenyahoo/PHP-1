...
INNER JOIN hospital on hospital.id=HospitalID WHERE id=0";
...
INNER JOIN hospital on hospital.id=HospitalID WHERE patientID='$id'";
    ...
    while ($row = $results->fetch(PDO::FETCH_ASSOC))
    {
        ...
        if($_SESSION["type"]=="Hospital")
        {
            ...
            $op='<tr '.$t.'><td>'.$row["PName"].'</td><td>'.$row["Status"].'</td><td>'.$row["PDate"].'</td><td>'.$row["PTimen"].':00</td><td>'.$d.'</td>
        </tr>';
        }else{
            if($_SESSION["type"]=="Patient")
            {
                $d=' <a class="btn btn-sm" href="../included/AppointmentEdit.php?aID='.$row["id"].'">Cancel</a>';
                $op='<tr '.$t.'><td>'.$row["HName"].'</td><td>'.$row["Status"].'</td><td>'.$row["PDate"].'</td><td>'.$row["PTimen"].':00</td><td>'.$d.'</td>
        </tr>';
            }
        }
        echo $op;
